...
def main(arguments=None):
    parser=ArgumentParser( description='Trending and a half', formatter_class=RawTextHelpFormatter)
    parser.add_argument('--leverage', help="Create demo files", action="store",default='10')
    parser.add_argument('--price', help="Initial price", action="store",required=True)
    parser.add_argument('--long', help="Long shares", action="store",default='0')
    parser.add_argument('--short', help="Create demo files", action="store", default='0')
    parser.add_argument('--percentage', action="Step percentage", default="30")

    args=parser.parse_args(arguments)
    
    price=Decimal(args.price)
    leverage=Decimal(args.leverage)
    long=Decimal(args.long)
    short=Decimal(args.short)
    percentage=Percentage(args.percentage, 1)
