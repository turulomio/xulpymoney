...
class Product(QObject):
    ...
    def remove(self):     
        if self.is_deletable()==True and self.is_system()==False:
            ...
            cur.execute("delete from estimations_dps where products_id=%s", (self.id, ))
            cur.execute("delete from estimations_eps where products_id=%s", (self.id, ))
...
class ProductManager(ObjectManager_With_IdName_Selectable, QObject):
    ...
    def move_data_between_products(self,  product_from,  product_to):
        ...
        cur.execute("update estimations_dps set id=%s where products_id=%s",(product_to.id,product_from.id))        
        cur.execute("update estimations_eps set id=%s where products_id=%s",(product_to.id,product_from.id))      
