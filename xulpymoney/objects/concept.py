...
class Concept:
    ...
    def save(self):
        if self.id==None:
            self.id=self.mem.con.cursor_one_field("insert into concepts (name, operationstypes_id, editable) values (%s, %s, %s) returning concepts_id", (self.name, self.tipooperacion.id, self.editable))
        else:
            self.mem.con.execute("update concepts set name=%s, operationstypes_id=%s, editable=%s where concepts_id=%s", (self.name, self.tipooperacion.id, self.editable, self.id))
    ...
    def uses(self):
        """Returns the number of uses in opercuenta and creditcardsoperations"""
        cur=self.mem.con.cursor()
        cur.execute("select count(*) from accountsoperations where concepts_id=%s", (self.id, ))
        accountsoperations=cur.fetchone()[0]
        cur.execute("select count(*) from creditcardsoperations where concepts_id=%s", (self.id, ))
        creditcardsoperations=cur.fetchone()[0]
        cur.close()
        return accountsoperations+creditcardsoperations

    def borrar(self):
        if self.is_deletable():
            cur=self.mem.con.cursor()        
            cur.execute("delete from concepts where id=%s", (self.id, ))
            cur.close()
            return True
        return False
    ...
    def media_mensual(self):
        cur=self.mem.con.cursor()
        cur.execute("select datetime from accountsoperations where concepts_id=%s union all select datetime from creditcardsoperations where concepts_id=%s order by datetime limit 1", (self.id, self.id))
        res=cur.fetchone()
        if res==None:
            primerafecha=date.today()-timedelta(days=1)
        else:
            primerafecha=res['datetime'].date()
        cur.execute("select sum(amount) as suma from accountsoperations where concepts_id=%s union all select sum(amount) as suma from creditcardsoperations where concepts_id=%s", (self.id, self.id))
        suma=Decimal(0)
        for i in cur:
            if i['suma']==None:
                continue
    ...
    def mensual(self,   year,  month):  
        """Saca el gasto mensual de este concept"""
        cur=self.mem.con.cursor()
        cur.execute("select sum(amount) as suma from accountsoperations where concepts_id=%s and date_part('month',datetime)=%s and date_part('year', datetime)=%s union all select sum(amount) as suma from creditcardsoperations where concepts_id=%s  and date_part('month',datetime)=%s and date_part('year', datetime)=%s", (self.id,  month, year,  self.id,  month, year  ))
        suma=0
        for i in cur:
            if i['suma']==None:
                continue
...
class ConceptManager(ObjectManager_With_IdName_Selectable, QObject):
    ...
    def percentage_monthly(self, year, month):
        ...
        1) Concept:
        2) Monthly expense
        3) Percentage expenses of all concepts this month
        4) Monthly average from first operation
        
        Returns three fields:
        1) dictionary arr, whith above values, sort by concept.name
        2) total expenses of all concepts                            
        3) total average expenses of all concepts
        """
...
def Concept_from_dict(mem, row):
    tipooperacion=mem.tiposoperaciones.find_by_id(row['operationstypes_id'])
    return Concept(mem, row['name'], tipooperacion, row['editable'], row['id'])
...
def ConceptManager_for_accountsoperations(mem):
        r=ConceptManager(mem)
...
def ConceptManager_by_operationtype(mem, operationstypes_id):
        resultado=ConceptManager(mem)
        for c in mem.concepts:
            if c.tipooperacion.id==operationstypes_id:
                resultado.append(c)
        return resultado
