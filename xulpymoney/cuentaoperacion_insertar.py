...
function insert(){
    ...
    var url="ajax/cuentaoperacion_insertar.py?fecha="+fecha+"&id_conceptos="+id_conceptos+"&id_tiposoperaciones="+id_tiposoperaciones+"&importe="+importe+"&comentario="+comentario+"&id_cuentas="+id_cuentas;
    xmlHttp=new XMLHttpRequest();
    xmlHttp.onreadystatechange=function(){
        if(xmlHttp.readyState==4){
            var ale=xmlHttp.responseText;
            location="cuentaoperacion_listado.py?id_cuentas="+ id_cuentas + "&year=" + spfecha[0] + "&month=" + spfecha[1];
        }
    }
...
function check_data(){
    ...
    if (/[\'\"]/.test(document.getElementById("comentario").value)){//Error gettext "'
...
<grid pack="center">
    ...
    <rows>
        <row>
            ...
            <datepicker id="fecha" type="grid"  firstdayofweek="1" value="<%=str(year)+'-'+str(month)+'-'+str(day)%>"/>
        </row>
        <row>
        ...
        <row>
            ...
            <textbox id="comentario" value="<%=comentario %>"/>
        </row>
    </rows>
</grid>
