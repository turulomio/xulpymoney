...
DROP FUNCTION public.investment_operations(p_investment_id integer, p_at_datetime timestamp with time zone, user_currency text );
...
CREATE OR REPLACE FUNCTION public.investment_operations(
    ...
    user_currency text,
    tablename text default 'investmentsoperations')
...
cur=[]
hist=[]
plan=plpy.prepare(f'SELECT * from {tablename} where investments_id=$1 and datetime <=$2  order by datetime', ["integer","timestamp with time zone"])
