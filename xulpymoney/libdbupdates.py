...
CREATE TABLE high_low_contract (
    ...
    CONSTRAINT high_low_contract_fk_products_id FOREIGN KEY (products_id) REFERENCES products (id) MATCH SIMPLE  ON UPDATE NO ACTION ON DELETE RESTRICT
        ...
        if self.dbversion<201812070708:#Update user products high_low=false
            cur=self.mem.con.cursor()
            cur.execute("update products set high_low=false where id<0")
            cur.close()
            self.mem.con.commit()
            self.set_database_version(201812070708)
        if self.dbversion<201812071223:#Update user products high_low=false
            cur=self.mem.con.cursor()
            cur.execute("ALTER TABLE high_low_contract DROP CONSTRAINT high_low_contract_fk_products_id")
            cur.execute("ALTER TABLE high_low_contract RENAME products_id TO investments_id")           
            cur.execute("ALTER TABLE high_low_contract ADD CONSTRAINT high_low_contract_fk_investments_id FOREIGN KEY (investments_id) REFERENCES inversiones (id_inversiones) MATCH SIMPLE  ON UPDATE NO ACTION ON DELETE RESTRICT")
            cur.close()
            self.mem.con.commit()
            self.set_database_version(201812071223)
        if self.dbversion<201812071303:#Update user products high_low=false
            cur=self.mem.con.cursor()
            cur.execute("ALTER TABLE high_low_contract ADD COLUMN guarantee_ao integer")
            cur.execute("ALTER TABLE high_low_contract ADD COLUMN adjustment_ao integer")
            cur.execute("ALTER TABLE high_low_contract ADD COLUMN comission_ao integer")
            cur.execute("ALTER TABLE high_low_contract ADD COLUMN interest_ao integer")  
            cur.execute("ALTER TABLE high_low_contract ADD CONSTRAINT high_low_contract_fk_guarantee_ao FOREIGN KEY (guarantee_ao) REFERENCES opercuentas (id_opercuentas) MATCH SIMPLE  ON UPDATE NO ACTION ON DELETE RESTRICT")
            cur.execute("ALTER TABLE high_low_contract ADD CONSTRAINT high_low_contract_fk_adjustment_ao FOREIGN KEY (adjustment_ao) REFERENCES opercuentas (id_opercuentas) MATCH SIMPLE  ON UPDATE NO ACTION ON DELETE RESTRICT")
            cur.execute("ALTER TABLE high_low_contract ADD CONSTRAINT high_low_contract_fk_comission_ao FOREIGN KEY (comission_ao) REFERENCES opercuentas (id_opercuentas) MATCH SIMPLE  ON UPDATE NO ACTION ON DELETE RESTRICT")
            cur.execute("ALTER TABLE high_low_contract ADD CONSTRAINT high_low_contract_fk_interest_ao FOREIGN KEY (interest_ao) REFERENCES opercuentas (id_opercuentas) MATCH SIMPLE  ON UPDATE NO ACTION ON DELETE RESTRICT")
            cur.close()
            self.mem.con.commit()
        ...
        if self.dbversion<201812110601:
            cur=self.mem.con.cursor()
            cur.execute("ALTER TABLE opportunities RENAME price TO entry")
            cur.execute("ALTER TABLE opportunities ADD COLUMN target numeric(100,2)")
            cur.execute("ALTER TABLE opportunities ADD COLUMN stoploss numeric(100,2)")
            cur.execute("ALTER TABLE opportunities ADD COLUMN pci character(1) default 'c'::bpchar")
            cur.close()
            self.mem.con.commit()
        ...
        if self.dbversion<201902140545:#Add global to control products.xlsx update. Added short field to opportunities
            cur=self.mem.con.cursor()
            
            cur.execute("drop FUNCTION public.last_penultimate_lastyear(INOUT id integer, OUT last_datetime timestamp with time zone, OUT last numeric, OUT penultimate_datetime timestamp with time zone, OUT penultimate numeric, OUT lastyear_datetime timestamp with time zone, OUT lastyear numeric)")
            cur.execute("""
CREATE FUNCTION public.last_penultimate_lastyear(INOUT id integer, IN at_datetime timestamp with time zone, OUT last_datetime timestamp with time zone, OUT last numeric, OUT penultimate_datetime timestamp with time zone, OUT penultimate numeric, OUT lastyear_datetime timestamp with time zone, OUT lastyear numeric) RETURNS record
    LANGUAGE plpgsql
    AS $$
DECLARE
    ly timestamptz;
...
END;
$$;""")
            cur.execute("ALTER FUNCTION public.last_penultimate_lastyear(INOUT id integer, IN at_datetime timestamp with time zone, OUT last_datetime timestamp with time zone, OUT last numeric, OUT penultimate_datetime timestamp with time zone, OUT penultimate numeric, OUT lastyear_datetime timestamp with time zone, OUT lastyear numeric) OWNER TO postgres;")
            cur.close()
            self.mem.con.commit()
        ...
        if self.dbversion<201908221326:
            cur=self.mem.con.cursor()
            cur.execute("alter table products add column decimals integer default 2")
            cur.execute("alter table orders drop column amount")
            cur.close()
            self.mem.con.commit()
