...
class wdgJointReport(QWidget, Ui_wdgJointReport):
    ...
    def load_rendimientos_less(self):
        """Rendimientos cuando contabilizan distinto las ganancias a menos de un año"""
        ...
        self.totalBruto=self.totalBrutoLess+self.totalBrutoMore
        self.totalComisiones=self.totalComisionesLess+self.totalComisionesMore
        self.totalImpuestos=self.totalImpuestosLess+self.totalImpuestosMore
        self.totalNeto=self.totalNetoLess+self.totalNetoMore
        
        if self.totalBruto>0:
            impxplus=-self.totalBrutoLess*self.mem.taxcapitalappreciationbelow-self.totalBrutoMore*self.mem.taxcapitalappreciation
        else:            
            impxplus=0
    ...
    def load_rendimientos(self):
        """Rendimientos cuando  no contabilizan distinto las ganancias  de un año"""
        inicio=datetime.date(self.wy.year-1, 12, 31)
        cur=self.mem.con.cursor()
        cur.execute("select sum(importe) as suma from opercuentas where id_conceptos=59 and date_part('year',datetime)="+str(self.wy.year))
        sumcomisioncustodia=cur.fetchone()[0]        
