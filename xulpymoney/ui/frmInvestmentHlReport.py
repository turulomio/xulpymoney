...
from xulpymoney.ui.Ui_frmInvestmentHlReport import Ui_frmInvestmentHlReport
...
class frmInvestmentHlReport(QDialog, Ui_frmInvestmentHlReport):
    ...
    def on_actionDisReinvest_triggered(self):
        d=QDialog()       
        d.resize(self.mem.settings.value("frmInvestmentHlReport/qdialog_disreinvest", QSize(1024, 768)))
        d.setWindowTitle(self.tr("Divest / Reinvest simulation"))
        w=wdgDisReinvest(self.mem, self.investment, False, d)
        lay = QVBoxLayout(d)
        lay.addWidget(w)
        d.exec_()
        self.mem.settings.setValue("frmInvestmentHlReport/qdialog_disreinvest", d.size())
