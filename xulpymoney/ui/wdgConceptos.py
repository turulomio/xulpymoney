...
class wdgConceptos(QWidget, Ui_wdgConceptos):
    ...
    def load_gastos(self,  year,  month):
        con=self.cfg.connect_xulpymoney()
        cur = con.cursor()
        gastos=Patrimonio().saldo_por_tipo_operacion(cur, year,  month, 1)
        conceptos=self.cfg.conceptos_tipooperacion(1)
        ...
        for i, c in enumerate(conceptos):
            ...
            self.tblGastos.setItem(i, 4, self.cfg.localcurrency.qtablewidgetitem(c.media_mensual(cur)))
        cur.close()     
        self.cfg.disconnect_xulpymoney(con)         
    ...
    def load_ingresos(self,  year,  month):
        con=self.cfg.connect_xulpymoney()
        cur = con.cursor()
        ingresos=Patrimonio().saldo_por_tipo_operacion(cur, year,  month, 2)
        conceptos=self.cfg.conceptos_tipooperacion(2)
        ...
        for i, c in enumerate(conceptos):
            ...
            self.tblIngresos.setItem(i, 4, self.cfg.localcurrency.qtablewidgetitem(c.media_mensual(cur)))
        cur.close()     
        self.cfg.disconnect_xulpymoney(con)         
