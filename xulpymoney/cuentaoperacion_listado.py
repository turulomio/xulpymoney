...
function borrar(){
    ...
    xmlHttp.onreadystatechange=function(){
        if(xmlHttp.readyState==4){
            ...
            location="cuentaoperacion_listado.psp?id_cuentas="+ id_cuentas + "&year=" + spfecha[0] + "&month=" + spfecha[1];
        }
    }
...
function update_list(){
    ...
    location="cuentaoperacion_listado.psp?id_cuentas="+ cmbcuentas + "&year=" +year + "&month="+ month;
}
...
<vbox align="center">
   ...
   <%=cmbcuentas%>
   <datepicker id="fecha" type="grid"  firstdayofweek="1" align="center" onchange="update_list();" value="<%=str(year)+'-'+str(month)+'-'+str(day)%>"/>
</vbox>
   <%=listado%>
