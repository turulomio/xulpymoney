...
class Dolthub(Command):
    description = "Update tables from dolthub"
    user_options = [
      # The format is (long option, short option, description).
      ( 'user=', None, 'Database user'),
      ( 'db=', None, 'Database name'),
    ...
    def run(self):
        ...
        con.execute("update products set obsolete=true where id>0")
        ...
        for j in tqdm(json):
            row=con.cursor_one_row("select * from products where id = %s", (j["id"],))
            if row is None:
                con.execute("""
                    INSERT INTO 
                        PRODUCTS
