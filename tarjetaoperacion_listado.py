...
function borrar(){
    ...
    var url='ajax/tarjetaoperacion_borrar.py?id_opertarjetas=' + id_opertarjetas ;
    xmlHttp=new XMLHttpRequest();
    xmlHttp.onreadystatechange=function(){
        if(xmlHttp.readyState==4){
            var ale=xmlHttp.responseText;
            location="tarjetaoperacion_listado.py?id_tarjetas="+ id_tarjetas;
        }
    }
...
function go_opertarjetas_insertar(){
    var id_tarjetas=<%=form['id_tarjetas']%>;
    location="tarjetaoperacion_insertar.py?id_tarjetas="+ id_tarjetas ;
}
...
function pagar(){
    ...
    var url='ajax/tarjeta_pagar.py?lista=' + lista + '&suma='+suma + '&id_tarjetas='+ id_tarjetas + '&fechapago='+ fechapago;
    xmlHttp=new XMLHttpRequest();
    xmlHttp.onreadystatechange=function(){
        if(xmlHttp.readyState==4){
            var ale=xmlHttp.responseText;
...
<popupset>
    <popup id="treepopup" >
        ...
        <menuitem label="Modificar la operación de tarjeta"  oncommand='location="tarjetasoperacion_modificar.py?id_cuentas=" + idcuenta  + "&amp;ibm=modificar&amp;regresando=0";'   class="menuitem-iconic"  image="images/toggle_log.png"/>
        <menuitem label="Borrar la operación de tarjeta"  oncommand='borrar();'  class="menuitem-iconic" image="images/eventdelete.png"/>
        <menuseparator/>
        <menuitem label="Realizar un pago"  oncommand="location='cuentasinformacion.py?id_cuentas=' + idcuenta;"/>
    </popup>
</popupset>
