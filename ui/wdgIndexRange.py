        ...
        def inversiones(arr,min,max):
            ...
            for i in arr:
                if i[0]>=min and i[0]<max:
                    o=i[1]
                    resultado=resultado+ self.tr("{0} {1} ({2}): {3} shares of {4} = {5}\n".format(str(o.datetime)[:16], o.investment.name, o.investment.account.name, round(o.acciones, 0),  o.investment.product.currency.string(o.valor_accion), o.investment.product.currency.string(o.importe)))
            return resultado[:-1]
        ######################################################
